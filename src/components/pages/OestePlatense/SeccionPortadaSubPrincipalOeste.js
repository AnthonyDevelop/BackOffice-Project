...
export default function SeccionPortadaSubPrincipalOeste(props) {
    ...
    const vistaMobile = useMediaQuery({
      ...
      for (let i = 1; i < 3; i++) {
        ...
        arregloSubprincipal.push(
          <Link to={"/suplemento/op/" + data[i].path} onClick={() => {localStorage.setItem("dataPostNota", JSON.stringify(data[i]))}} >
        <div className={i==2 && !vistaMobile ? 'box-portada-subprincipal child-card-borde-oesteplatense'  : i==2 && vistaMobile ? 'box-portada-subprincipal child-card-borde-top' : 'box-portada-subprincipal'} >
        <div className='box-portada-subprincipal-image-secction'>
            <div className='container-post-categoria-oeste'> 
            {(data[i].categorias[0].nombre=="DESTACADO_PPAL" && data[i].categorias[1].nombre=="DESTACADO_VIEW") ? data[i].categorias[2].nombre : (data[i].categorias[0].nombre=="DESTACADO_PPAL" || data[i].categorias[0].nombre=="DESTACADO_VIEW") ? data[i].categorias[1].nombre :  data[i].categorias[0].nombre}
